...
;; ;; compatibility for python-mode package
;; (unless (boundp 'py-eldoc-function)
;;   (setq py-eldoc-function 'python-eldoc-function))
;;====================== exec current buffer =============
...
(defun my-elpy/execute-from-project-root (file root)
  ...
  (let* ((file (expand-file-name file))
	 ...
	 (command (format "python -m %s " module-name))
	 )
...
(define-key python-mode-map (kbd "C-c r") 'my-elpy/execute-buffer)
...
(if  (eq system-type 'windows-nt)
    ...
    (setq python-my-virtualenv-prefix "c:/soft/miniconda3/envs")
  )
;; (pyvenv-activate (concat python-my-virtualenv-prefix "/py37"))

(defun myactivate (env-name)
  (interactive "s")
  (pyvenv-activate (concat python-my-virtualenv-prefix "/" env-name)))
